...
contract RestaurantManager is Ownable {
    ...
    struct Drop {
        ...
        uint16 royaltiesBps; //basis points for royalty percentage
        address royaltiesReceiver; //address to receive royalties
        bool isActive;
    }
    ...
    function getRestaurant(
        ...
        if (restaurants[restaurantId].owner == address(0)) {
    ...
    function getDropIDsByRestaurant(
        ...
        for (uint i = 0; i < _dropCounter; i++) {
          if (drops[i + 1].restaurantId == restaurantId) {
              // NOTE: drop IDs indices start from 1
              result[counter] = i + 1;
              counter++;
          }
        }
