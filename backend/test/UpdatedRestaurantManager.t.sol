...
contract RestaurantManagerTest is Test {
    ...
    function createSampleDrop()
        ...
        returns (
            ...
            uint16 royaltiesBps, //adding royalty parameters
        ...
        )
    {
        ...
        uint16 royaltiesBps = 100; // 1% royalty
        address royaltiesReceiver = address(this); //using the test contract address for now
    ...
    function test_CreateDropWithHourlyWindowsAndRoyalties() public {
        uint256 restaurantId = registerSampleRestaurant();
        (
            uint64 startDate,
            uint64 endDate,
            ...
            uint32 windowDuration,
            uint16 reservationsPerWindow,
            uint16 royaltiesBps,    //added royalty parameters in the test
            address royaltiesReceiver
        ) = createSampleDrop();
        ...
        // New assertions for royalty data
        assertEq(drop.royaltiesBps, royaltiesBps, "Royalties basis points should match");
        assertEq(drop.royaltiesReceiver, royaltiesReceiver, "Royalties receiver should match");
