...
contract MarketplaceTest is Test {
    ...
    function setUp() public {
        ...
        uint16 royaltiesBps = 100; // added a 1% royalty
        address royaltiesReceiver = address(this); // test contract address for meantime
    ...
    function test_BuyReservationWithRoyaltyPayment() public payable {
    uint256 tokenId = 1; 
    uint256 salePrice = 0.02 ether;

    // List the NFT for sale
    marketplace.listReservation(tokenId, salePrice);

    // Record balances before the sale
    uint256 royaltyReceiverInitialBalance = address(royaltiesReceiver).balance;
    uint256 sellerInitialBalance = address(this).balance;

    // Simulate a buyer purchasing the NFT
    marketplace.buyReservation{value: salePrice}(tokenId);

    // Calculate expected royalty
    uint256 expectedRoyalty = salePrice * royaltiesBps / 10000;

    // Assertations to check that the royalty payment was correctly processed
    assertEq(address(royaltiesReceiver).balance, royaltyReceiverInitialBalance + expectedRoyalty, "Royalty receiver should receive the correct royalty amount");
    assertEq(address(this).balance, sellerInitialBalance + salePrice - expectedRoyalty, "Seller should receive sale price minus royalty");
}
